...
Способ доставки — справочный домен, отдельная сущность логистики.
- HUB_SEGMENT — бизнес‑ключ: Segment
Сегмент клиента — независимый классификатор спроса.
- HUB_LOCATION — бизнес‑ключ: конкатенация Country|State|City|Postal Code|Region
Геоконтекст продаж. В этой работе объединил в один ключ,чтобы не плодить иерархию Country/State/City; в теории так сделать можно.
- HUB_PRODUCT_CATEGORY — бизнес‑ключ: Category|Sub-Category
Товарная классификация в источнике представлена именно так.

**Линки (Links):**
- LNK_SALE_CTX — связь «контекст продажи»: Ship Mode ↔ Segment ↔ Location ↔ Product Category.
Чтобы показать целостность доменов в одном «контексте продажи» сделал 1 линк. В принципе можно разбить на 3–4 отдельные связи (например, SALE ↔ SEGMENT, SALE ↔ LOCATION, SALE ↔ PRODUCT_CATEGORY, SALE ↔ SHIP_MODE) для простоты от этого решил отказаться. На реальных проектах, понятное дело, такого бы сделать не дали.
...
Все сателлиты будут строится согласно SCD2

**Общие правила**
Hash-keys: все hub/link ключи = md5(upper(trim(concat_ws('|', ...)))) → стабильная детерминированность, быстрые join-ы, независимость от источников.
- Hashdiff в сателлитах: хэш от атрибутов версии строки → детект изменений и историзация.
- Ссылочная целостность: фактически обеспечивается в ETL (детерминированные ключи). Для ER-диаграммы добавлены логические FK (NOT VALID), чтобы визуально показать связи, не тормозя загрузку.
- Техполя: load_dts, record_source - трассируемость происхождения и времени загрузки.
- Greenplum: DISTRIBUTED BY (…_key) - локальность данных при join-ах, масштабируемые вставки
...
PS. Все SQL запросы представлены здесь [тут](https://github.com/andrey-osadchiy/data_storage/tree/main/final/SQL/sql.sql)
