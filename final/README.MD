...
**Общие правила**
- Hash-keys: все hub/link ключи = md5(upper(trim(concat_ws('|', ...))))
- Hashdiff в сателлитах: хэш от атрибутов версии строки → детект изменений и историзация.
- Ссылочная целостность: фактически обеспечивается в ETL (детерминированные ключи). Также создадим логические FK (NOT VALID), чтобы визуально показать связи, не тормозя загрузку.
- Создадим в таблицах техполя: load_dts, record_source
